...
class HuffmanSuite extends FunSuite {
  ...
  test("ordered leaf list") {
    assert(makeOrderedLeafList(List(('b', 3), ('a', 2))) == List(Leaf('a',2),Leaf('b',3)))
  }
  ...
  test("until combine of some leaf list") {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 4))
  ...
  test("chartobitstring") {
    new TestTrees {
      assert(charToBitstring('b', t2, List()) === List(0, 1))
    }
  }
